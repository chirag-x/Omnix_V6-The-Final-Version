...
# Import the test function and run it with extra debugging
from tests.test_stage18_7_perception_bridge import test_perception_strategy_ranking_respected

# Let's also import what we need to manually run the test
...
print("Creating mock strategies...")
...
print("Router created with strategies:", [s.source_type.value for s in router.strategies])

resolver = TargetResolver()
bridge = PerceptionToGroundingBridge(router=router, resolver=resolver)

print("Bridge created")
print(f"Bridge router strategies count: {len(bridge.router.strategies)}")

print("Calling ground_target...")
...
print(f"Expected status: {TargetResolutionStatus.RESOLVED}")
print(f"Actual status: {result.status}")
print(f"Match: {result.status == TargetResolutionStatus.RESOLVED}")